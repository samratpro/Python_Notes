...
sudo bash install_7.0_en.sh aapanel -y
...
# create .gitignore  with proper ignore all
...
### 04. Git ignore
```
/20d7feb57b7821c42b92394d74f29c7f_venv
...
```
### Project fail reasones
- If TCP/UDP rule added from security for 8000 port or target installing port can fail project
- Permission issue, change user as root then change permission
### Terminal Base Runs
```
sudo su - # as a root user
chmod -R 777 /www/wwwroot
sudo chown -R www:www /www/wwwroot/
sudo chmod 755 /www/wwwroot/
sudo chmod 644 /www/wwwroot/
# if fail ----i---------e------- /www/wwwroot/domain.com/.user.ini
chattr -i /www/wwwroot/domain.com/.user.ini
## server run from terminal if fail
ps aux | grep gunicorn # check status
nohup sudo -u www /www/wwwroot/service_site/000000007f_venv/bin/gunicorn --bind 127.0.0.1:8000 core.wsgi:application > /tmp/gunicorn.log 2>&1 &
```
...
### Domain / Config / Config File -> bottom
```
errorpage 404 {
  url /index.php
}
```
### Restart Openlitespeed  from app store
