| Section | Description | Where to Click/Scroll | Details |
|---------|-------------|----------------------|---------|
| **01. Using Cookie Example Code with Chromium/Custom Chrome** | Saves and reuses session cookies for LinkedIn login. | **Click**: Press Enter in the terminal/console. | After logging in manually on the LinkedIn page, press Enter to save the session state (cookies) to `storage_state.json`. No scrolling required. |
| **1.1 Example Login with Gmail** | Automates Gmail login on Indeed using Google button. | **Click**: Google login button (`#login-google-button`).<br>**Click**: Press Enter in the terminal/console. | Clicks the Google login button to open a new tab for authentication. After manual login, press Enter to save the session. No scrolling required. |
| **02. Work with Real Browser** | Controls a real Chrome browser instance via remote debugging. | **Click**: Press Enter in the terminal/console. | After navigating to Facebook, press Enter to close the browser. No specific clicking or scrolling on the page itself. |
| **03. Cookie Use with Real Chrome Browser** | Manages Chrome with remote debugging and saves/reuses session for Pinterest. | **Click**: Press Enter in the terminal/console. | After navigating to Pinterest and logging in (if needed), press Enter to save the session or close the browser. No scrolling required. |
| **04. Fixed 3 Tab Async** | Fetches URLs concurrently using 2 browser tabs. | No explicit clicking or scrolling. | Automatically navigates to URLs in `error_list` using two tabs. No user interaction (click/scroll) required during execution. |
| **05. Limit Concurrency 3 Tab Async** | Limits concurrency to 2 tabs for visiting URLs. | No explicit clicking or scrolling. | Automatically visits URLs in `error_list` with a semaphore limiting to 2 concurrent tabs. No user interaction required. |
| **06. Example Amazon Product Scraping with Multi Tab** | Scrapes Amazon product data using multiple tabs. | No explicit clicking or scrolling. | Automatically navigates to search results and product pages. No user interaction (click/scroll) required during scraping. |
| **07. Work with Custom Browser** | Uses a custom Chrome executable to save/reuse Pinterest session. | **Click**: Press Enter in the terminal/console. | After manual login on Pinterest, press Enter to save the session state. No scrolling required. |
