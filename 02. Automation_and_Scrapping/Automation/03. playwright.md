...
page.evaluate("""document.getElementById('searchbox-searchbutton').click()""")
...
# mouse move and click like real
# mouse move
btn_box = scraper_page.locator("[_path]").first.bounding_box()
x = btn_box["x"] + btn_box["width"]  / 2 + random.uniform(-5, 5)
y = btn_box["y"] + btn_box["height"] / 2 + random.uniform(-5, 5)
scraper_page.mouse.move(x, y, steps=random.randint(8, 15))
time.sleep(random.uniform(0.2, 0.5))
# real JS click
scraper_page.evaluate('''
    () => {
        const btn = document.querySelector("[query_selector_path]");
        if (btn) {
            const down = new MouseEvent('mousedown', {bubbles:true, cancelable:true, clientX:%s, clientY:%s});
            const up   = new MouseEvent('mouseup',   {bubbles:true, cancelable:true, clientX:%s, clientY:%s});
...
# Advance click
phone_ele = scraper_page.locator("div[data-tooltip-place='bottom'] button[title='voir le numéro']").first
if phone_ele.count() > 0:
    # Human-like interaction
    box = phone_ele.bounding_box()
    if box:
        x = box["x"] + box["width"] / 2 + random.uniform(-5, 5)
        y = box["y"] + box["height"] / 2 + random.uniform(-5, 5)
        scraper_page.mouse.move(x, y, steps=random.randint(8, 15))
        time.sleep(random.uniform(0.2, 0.5))
        phone_ele.click(delay=random.randint(100, 200))
```
...
phone_elem = page.locator("//a[@title='appeler']").first
phone_elem.wait_for(state="visible", timeout=15000)
``` 
## 25. scrolling
```py
scroll=0
