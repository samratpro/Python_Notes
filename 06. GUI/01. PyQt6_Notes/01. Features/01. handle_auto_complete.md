...
class SellerApp(QWidget):
    def __init__(self):
        ...
        self.engine = create_engine('sqlite:///sellers.db')
        ...
        # Autocomplete for seller names
        self.sellers_name = self.get_seller_names()
        self.completer = QCompleter(self.sellers_name, self)
        self.completer.setCaseSensitivity(Qt.CaseSensitivity.CaseInsensitive)  # Case-insensitive matching
        self.seller_input.setCompleter(self.completer)
    ...
    def get_seller_names(self):
        """Fetch all seller names from the database for autocomplete."""
        self.session = self.Session()
        self.sellers = self.session.query(Seller).all()
        self.session.close()
    ...
    def seller_exists(self, name):
        ...
        exists = session.query(Seller).filter(Seller.name == name).first() is not None
        session.close()
        return exists
